-- FulQrun Database Schema for MVP - FIXED VERSION
...
-- First, create a function to get the user's organization ID
...
-- Users policy - users can only see users from their organization
CREATE POLICY "Users can only see their organization's users" ON users
    FOR ALL USING (organization_id = get_user_organization_id());

-- Companies policy - users can only see companies from their organization
CREATE POLICY "Users can only see their organization's companies" ON companies
    FOR ALL USING (organization_id = get_user_organization_id());

-- Contacts policy - users can only see contacts from their organization
CREATE POLICY "Users can only see their organization's contacts" ON contacts
    FOR ALL USING (organization_id = get_user_organization_id());

-- Leads policy - users can only see leads from their organization
CREATE POLICY "Users can only see their organization's leads" ON leads
    FOR ALL USING (organization_id = get_user_organization_id());

-- Opportunities policy - users can only see opportunities from their organization
CREATE POLICY "Users can only see their organization's opportunities" ON opportunities
    FOR ALL USING (organization_id = get_user_organization_id());

-- Activities policy - users can only see activities from their organization
CREATE POLICY "Users can only see their organization's activities" ON activities
    FOR ALL USING (organization_id = get_user_organization_id());

-- Lead scoring rules policy - users can only see rules from their organization
CREATE POLICY "Users can only see their organization's scoring rules" ON lead_scoring_rules
    FOR ALL USING (organization_id = get_user_organization_id());

-- Pipeline stages policy - users can only see stages from their organization
